...
export default function ContactPage() {
  ...
  return (
    <div className="pt-24">
      ...
      <section className={`py-20 ${dk ? 'bg-gradient-to-b from-dark-900 to-dark-950' : 'bg-gradient-to-b from-dark-50 to-white'}`}>
        <div className="max-w-7xl mx-auto px-4 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <h1 className={`text-4xl sm:text-5xl font-display font-bold mt-2 ${dk ? 'text-white' : 'text-dark-900'}`}>Contact Us</h1>
            <p className={`mt-4 max-w-2xl mx-auto ${dk ? 'text-dark-400' : 'text-dark-500'}`}>Have a question? We'd love to hear from you.</p>
          </motion.div>
        </div>
      </section>
      ...
      <section className={`py-16 ${dk ? 'bg-dark-900' : 'bg-white'}`}>
        <div className="max-w-7xl mx-auto px-4 grid lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-8">
            <h2 className={`text-2xl font-display font-bold ${dk ? 'text-white' : 'text-dark-900'}`}>Reach Out Directly</h2>
            <div className="space-y-6">
              {[
                ...
                <a key={item.label} href={item.href} target={item.href.startsWith('http') ? '_blank' : undefined} rel="noopener noreferrer"
                  className={`flex items-start gap-4 p-4 rounded-xl transition-all ${dk ? 'bg-dark-800/50 border border-dark-700/50 hover:border-gold-500/30' : 'bg-dark-50 border border-dark-100 hover:border-gold-500/30'}`}>
                  ...
                  <div>
                    <div className={`text-xs uppercase tracking-wider ${dk ? 'text-dark-500' : 'text-dark-400'}`}>{item.label}</div>
                    <div className={`text-sm mt-1 ${dk ? 'text-white' : 'text-dark-900'}`}>{item.value}</div>
                  </div>
                </a>
              ))}
            </div>
          ...
          <div className={`p-8 rounded-2xl ${dk ? 'bg-dark-800/50 border border-dark-700/50' : 'bg-dark-50 border border-dark-100'}`}>
            {submitted ? (
              <div className="text-center py-12">
                ...
                <h3 className={`text-xl font-bold mb-2 ${dk ? 'text-white' : 'text-dark-900'}`}>Message Sent!</h3>
                <p className={dk ? 'text-dark-400' : 'text-dark-500'}>We'll respond within 24 hours.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-5">
                <h3 className={`text-xl font-display font-bold mb-4 ${dk ? 'text-white' : 'text-dark-900'}`}>Send a Message</h3>
                ...
                <textarea rows={4} placeholder="Your Message" required value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} className={`${inputCls} resize-none`} />
      ...
      {/* Map */}
      <section className={dk ? 'bg-dark-950' : 'bg-dark-50'}>
