...
import { FaFileAlt, FaCheck, FaWhatsapp, FaMapMarkerAlt } from 'react-icons/fa';
import { endpoints } from '../api';
import { Link } from 'react-router-dom';
import type { Service } from '../types';

const STEPS = ['Your Details', 'Project Type', 'Specifications', 'Review & Submit'];
...
const inputCls =
  'w-full px-4 py-3 bg-dark-800 border border-dark-700 rounded-xl text-white placeholder-dark-500 focus:outline-none focus:border-gold-500 transition-colors';
const labelCls = 'block text-white text-sm font-medium mb-2';
const selectCls =
  'w-full px-4 py-3 bg-dark-800 border border-dark-700 rounded-xl text-white focus:outline-none focus:border-gold-500 transition-colors';
...
export default function QuotationPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await endpoints.createQuotation(payload);
      setSubmitted(true);
    } catch {
      ...
      <section className="py-12 sm:py-16 bg-dark-900">
        <div className="max-w-2xl mx-auto px-4">
          <AnimatePresence mode="wait">
              ...
              <motion.div key="s0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="space-y-5">
                ...
                <div>
                  ...
                  <div className="flex">
                    ...
                    <input type="tel" value={form.phone} onChange={(e) => update('phone', e.target.value)} placeholder="7258021382"
                      className={`${inputCls} rounded-l-none`} maxLength={10} />
                  </div>
            ...
            {step === 1 && (
              <motion.div key="s1" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="space-y-5">
                <h3 className="text-lg font-display font-semibold text-white mb-1">Project Type</h3>
              ...
              <motion.div key="s3" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
                ...
                <div className="bg-dark-800 border border-dark-700 rounded-2xl p-6 space-y-4">
                  ...
                  <div>
                    ...
                    {form.service && (() => { const s = services.find((x) => x.id === form.service); return s ? <p className="text-gold-400 text-sm">Service: {s.name}</p> : null; })()}
          ...
          <div className="flex gap-4 mt-10">
            {step > 0 && (
              <button onClick={prev}
                className="flex-1 py-3 bg-dark-800 text-white rounded-xl font-bold hover:bg-dark-700 transition-colors border border-dark-700">
                Previous
              </button>
            )}
            {step < 3 ? (
              <button onClick={next}
                className="flex-1 py-3 bg-gradient-to-r from-gold-500 to-gold-400 text-dark-900 rounded-xl font-bold hover:opacity-90 transition-opacity">
                Next Step
              </button>
            ) : (
